...
def _format_slack_message(event: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    text = f"{emoji} *cc-collab*: {event.replace('_', ' ').title()}"

    blocks = [
        {"type": "header", "text": {"type": "plain_text", "text": f"cc-collab: {event.replace('_', ' ').title()}"}},
...
def _format_discord_message(event: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    embed = {
        "title": f"cc-collab: {event.replace('_', ' ').title()}",
    ...
    return {"content": f"cc-collab: {event}", "embeds": [embed]}
