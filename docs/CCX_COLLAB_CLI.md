# ccx-collab CLI Reference

`ccx-collab` is a unified command-line interface for the Claude Code + Codex CLI collaboration pipeline. It replaces the shell-script-based workflow (`pipeline-runner.sh`, `orchestrate.py`) with a single, ergonomic CLI tool built on [Click](https://click.palletsprojects.com/) and [Rich](https://rich.readthedocs.io/).
...
```bash
ccx-collab validate --task agent/tasks/example.task.json --work-id demo --out results/validation.json
```

### Check pipeline progress

```bash
ccx-collab status --work-id my-feature
```

### Create a new task template

```bash
ccx-collab init --task-id FEAT-001 --title "Add authentication module"
```
...
ccx-collab -v --simulate run --task agent/tasks/example.task.json
...
```bash
# Basic validation
ccx-collab validate --task agent/tasks/example.task.json
...
ccx-collab run --task agent/tasks/example.task.json
...
ccx-collab health --out agent/results/health.json
```
...
ccx-collab cleanup --dry-run

# Delete files older than 7 days
ccx-collab cleanup --retention-days 7

# Clean a specific directory
ccx-collab cleanup --results-dir output/results --retention-days 14
```
...
ccx-collab init --task-id FEAT-042 --title "Implement caching layer"
...
**Note:** When using `ccx-collab --simulate`, the tool sets `SIMULATE_AGENTS=1` automatically. You do not need to set it manually.
...
# Just pass --simulate to ccx-collab
```
...
This section maps the legacy shell-script and `orchestrate.py` commands to their `ccx-collab` equivalents.
...
| Legacy Command (`orchestrate.py`)                    | ccx-collab Equivalent               |
|------------------------------------------------------|-------------------------------------|
| `python3 agent/scripts/orchestrate.py validate-task` | `ccx-collab validate`               |
| `python3 agent/scripts/orchestrate.py run-plan`      | `ccx-collab plan`                   |
| `python3 agent/scripts/orchestrate.py split-task`    | `ccx-collab split`                  |
| `python3 agent/scripts/orchestrate.py run-implement` | `ccx-collab implement`              |
| `python3 agent/scripts/orchestrate.py merge-results` | `ccx-collab merge`                  |
| `python3 agent/scripts/orchestrate.py run-verify`    | `ccx-collab verify`                 |
| `python3 agent/scripts/orchestrate.py run-review`    | `ccx-collab review`                 |
| `python3 agent/scripts/orchestrate.py run-retrospect`| `ccx-collab retrospect`             |
| `./agent/scripts/pipeline-runner.sh --task ...`      | `ccx-collab run --task ...`         |
| (no equivalent)                                       | `ccx-collab status --work-id ...`   |
| (no equivalent)                                       | `ccx-collab health`                 |
| (no equivalent)                                       | `ccx-collab cleanup`                |
| (no equivalent)                                       | `ccx-collab init`                   |

### Key differences

1. **Global options come first.** With `ccx-collab`, flags like `--simulate` and `--verbose` go before the subcommand:
   ```bash
   # Correct
   ccx-collab --simulate validate --task ...

   # Incorrect
   ccx-collab validate --simulate --task ...
   ```
...
3. **`--verify` is repeatable in review.** Pass multiple verification result files:
   ```bash
   ccx-collab review --verify file1.json --verify file2.json ...
   ```
