...
# Built-in defaults for ccx-collab configuration.
CCX_COLLAB_DEFAULTS: Dict[str, Any] = {
...
def load_ccx_collab_config(
    *,
    project_dir: Optional[Path] = None,
    user_dir: Optional[Path] = None,
    cli_overrides: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """Load and merge ccx-collab configuration from all sources.

    Precedence (highest to lowest):
        1. cli_overrides  -- explicit CLI flags
        2. project config -- .ccx-collab.yaml in the project root
        3. user config    -- ~/.ccx-collab/config.yaml
        4. CCX_COLLAB_DEFAULTS -- built-in defaults

    Parameters
    ----------
    project_dir:
        Directory to look for ``.ccx-collab.yaml``.  Defaults to
    ...
    merged: Dict[str, Any] = dict(CCX_COLLAB_DEFAULTS)

    # Layer 3: user-level config (~/.ccx-collab/config.yaml)
    if user_dir is None:
    ...
    # Layer 2: project-level config (.ccx-collab.yaml in project root)
    if project_dir is None:
        project_dir = get_project_root()
    project_config_path = project_dir / ".ccx-collab.yaml"
    ...
    logger.debug("Final merged ccx-collab config: %s", merged)
    return merged
