"""Integration tests for ccx-collab commands."""
...
from ccx_collab.config import CCX_COLLAB_DEFAULTS, load_ccx_collab_config
...
class TestConfigModule:
    ...
    def test_load_pipeline_config(self):
        from ccx_collab.config import load_pipeline_config
...
class TestOutputModule:
    ...
    def test_print_error_does_not_crash(self):
        ...
        from ccx_collab.output import print_error
        print_error("")
    ...
    def test_print_success_does_not_crash(self):
        ...
        from ccx_collab.output import print_success
        print_success("")
    ...
    def test_print_pipeline_header(self):
        ...
        from ccx_collab.output import print_pipeline_header
    ...
    def test_print_json_result(self):
        ...
        from ccx_collab.output import print_json_result
    ...
    def test_print_stage_result_with_output_path(self):
        ...
        from ccx_collab.output import print_stage_result
...
class TestCcxCollabConfigFile:
    """Tests for YAML config file loading and precedence."""

    def test_defaults_when_no_config_files(self, tmp_path):
        """With no config files present, load_ccx_collab_config returns defaults."""
        config = load_ccx_collab_config(
            project_dir=tmp_path,
            user_dir=tmp_path / "nonexistent_user_dir",
        )
        assert config["results_dir"] == CCX_COLLAB_DEFAULTS["results_dir"]
        assert config["retention_days"] == CCX_COLLAB_DEFAULTS["retention_days"]
        assert config["simulate"] == CCX_COLLAB_DEFAULTS["simulate"]
        assert config["verbose"] == CCX_COLLAB_DEFAULTS["verbose"]
        assert config["verify_commands"] == CCX_COLLAB_DEFAULTS["verify_commands"]

    def test_project_level_config(self, tmp_path):
        """Project-level .ccx-collab.yaml should override defaults."""
        project_cfg = {
            "results_dir": "custom/results",
            "retention_days": 7,
        }
        (tmp_path / ".ccx-collab.yaml").write_text(
    ...
    def test_user_level_config(self, tmp_path):
        """User-level ~/.ccx-collab/config.yaml should override defaults."""
        user_dir = tmp_path / "user_home" / ".ccx-collab"
    ...
    def test_precedence_project_over_user(self, tmp_path):
        ...
        project_dir.mkdir()
        (project_dir / ".ccx-collab.yaml").write_text(
    ...
    def test_yaml_with_non_dict_content(self, tmp_path):
        ...
        assert config == CCX_COLLAB_DEFAULTS
